...
import com.example.facedetection.Adapter.FaceDetectionAdapter;
        ...
        for (int i = 0; i < firebaseVisionFaces.size(); i++) {
            ...
            canvas.drawText("Face" + i, (firebaseVisionFaces.get(i).getBoundingBox().centerX()
                    ...
                    (firebaseVisionFaces.get(i).getBoundingBox().centerY() +
                            (firebaseVisionFaces.get(i).getBoundingBox().height() >> 1)) - 8f, facePaint);
            ...
            faceDetectionModels.add(new FaceDetectionModel(i, "Smiling Probability" + face.getSmilingProbability()));//0.99*100=%
            faceDetectionModels.add(new FaceDetectionModel(i, "Left Eye Opening Probability" + face.getLeftEyeOpenProbability()));
            faceDetectionModels.add(new FaceDetectionModel(i, "Right Eye Opening Probability" + face.getRightEyeOpenProbability()));

        }
    }

    private void showProgress() {
        findViewById(R.id.bottom_sheet_button_imageView).setVisibility(View.GONE);
        findViewById(R.id.bottom_sheet_button_progressbar).setVisibility(View.VISIBLE);
    }

    private void hideProgress() {
        findViewById(R.id.bottom_sheet_button_imageView).setVisibility(View.VISIBLE);
    ...
    public void process(@NonNull final Frame frame) {
     ...
     //   final int height = frame.getSize().getHeight();
        ...
        FirebaseVisionImageMetadata metadata = new FirebaseVisionImageMetadata.Builder()
                ...
                .setHeight(frame.getSize().getWidth())
                .setFormat(frame.getFormat())
                .setRotation((cameraFacing == Facing.FRONT) ? FirebaseVisionImageMetadata.ROTATION_270 : FirebaseVisionImageMetadata.ROTATION_90)
                .build();
        ...
        faceDetector.detectInImage(firebaseVisionImage).addOnSuccessListener(new OnSuccessListener<List<FirebaseVisionFace>>() {
            ...
            public void onSuccess(List<FirebaseVisionFace> firebaseVisionFaces) {
                ...
                Bitmap bitmap = Bitmap.createBitmap(frame.getSize().getWidth(), frame.getSize().getHeight(), Bitmap.Config.ARGB_8888);
                Canvas canvas = new Canvas(bitmap);
